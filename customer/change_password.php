	...
	if(isset($_SESSION['customer_login'])) {
		...
		if($_SERVER['REQUEST_METHOD'] == "POST") {
			if(!empty($_POST['txtOldPassword'])){
				...
				$query_oldPassword = "SELECT password FROM customer details WHERE cus_id='$id' AND password='$password'";
				...
				if($row_oldPassword) {
					if(!empty($_POST['txtNewPassword']) && !empty($_POST['txtConfirmPassword'])){
						...
						if(strcmp($newPassword,$confirmPassword) == 0) {
							$query_UpdatePassword = "UPDATE retailer SET password='$confirmPassword' WHERE cus_id='$id'";
