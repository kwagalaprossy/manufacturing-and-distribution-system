				...
				if(isset($_POST['cmbFilter'])) {
					if(!empty($_POST['txtInvoiceId'])) {
						...
						if($result == 1) {
							...
							$query_selectInvoice = "SELECT * FROM invoice,customer details,location WHERE invoice.cus_id=customer.cus_id AND customer.location=location.location AND invoice_id='$invoice_id' AND invoice.cus_id='$cus_id'";
						...
						if($result == 1) {
							...
							$query_selectInvoice = "SELECT * FROM invoice,customer details,location WHERE invoice.cus_id=customer.cus_id AND customer.location=location.location AND order_id='$order_id' AND invoice.cus_id='$cus_id'";
							$result_selectInvoice = mysqli_query($con,$query_selectInvoice);
							$row_selectInvoice = mysqli_fetch_array($result_selectInvoice);
							if(empty($row_selectInvoice)){
							   $error = "* No invoice was found with this ID";
