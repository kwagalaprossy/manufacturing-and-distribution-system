<?php
	...
	if(isset($_SESSION['customer_login'])) {
		if(isset($_SESSION['customer_login']) == true && isset($_SESSION['cus_id'])) {
			$retailer_id = $_SESSION['cus_id'];
			$error = "";
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				if(isset($_POST['cmbFilter'])) {
					if(!empty($_POST['txtId'])) {
						...
						if($result == 1) {
							...
							$query_selectOrder = "SELECT * FROM orders,customer details,location WHERE orders.cus_id=cus.cus_id AND customer.location=location.location AND order_id='$order_id' AND orders.retailer_id='$retailer_id'";
						...
						$date = $_POST['txtDate'];
						$query_selectOrder = "SELECT * FROM orders,customer details,location WHERE orders.cus_id=cus.cus_id AND customer.location=location.location AND date='$date' AND orders.cus_id='$cus_id'";
						$result_selectOrder = mysqli_query($con,$query_selectOrder);
						$row_selectOrder = mysqli_fetch_array($result_selectOrder);
						if(empty($row_selectOrder)){
						...
						$query_selectOrder = "SELECT * FROM orders,customer details,location WHERE orders.cus_id=cus.cus_id AND customer.location=location.location AND status='$status' AND orders.cus_id='$cus_id' ORDER BY approved,order_id DESC";
						$result_selectOrder = mysqli_query($con,$query_selectOrder);
						$row_selectOrder = mysqli_fetch_array($result_selectOrder);
						if(empty($row_selectOrder)){
						...
						$query_selectOrder = "SELECT * FROM orders,customer,area WHERE orders.cus_id=cus.cus_id AND customer.location=location AND approved='$approved' AND orders.cus_id='$cus_id' ORDER BY order_id DESC";
						$result_selectOrder = mysqli_query($con,$query_selectOrder);
						$row_selectOrder = mysqli_fetch_array($result_selectOrder);
						if(empty($row_selectOrder)){
						   $error = "* No order was found";
				...
				$query_selectOrder = "SELECT * FROM orders WHERE cus_id='$cus_id'";
				$result_selectOrder = mysqli_query($con,$query_selectOrder);
			}
		}
		else {
