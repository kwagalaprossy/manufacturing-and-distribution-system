<?php
	...
	if(isset($_SESSION['admin_login'])) {
		if($_SESSION['admin_login'] == true) {
			...
			$nameHolder = $emailHolder = $phoneHolder = $addressHolder = $usernameHolder = $passwordHolder = "";
			if($_SERVER['REQUEST_METHOD'] == "POST") {
				...
				if(!empty($_POST['txtDistributorUname'])) {
					$usernameHolder = $_POST['txtDistributorUname'];
					$resultValidate_username = validate_username($_POST['txtDistributorUname']);
					if($resultValidate_username == 1) {
						$username = $_POST['txtDistributorUname'];
					}
					else{
				...
				if(!empty($_POST['txtDistributorPassword'])) {
					$resultValidate_password = validate_password($_POST['txtDistributorPassword']);
				...
				if($name != null && $phone != null && $resultValidate_email ==1) {
					$query_addDistributor = "INSERT INTO distributor(dist_name,dist_email,dist_phone,dist_address,username,password) VALUES('$name','$email','$phone','$address','$username','$password')";
...
<body>
	...
	<div id="layout-wrapper">
		...
		<!-- ========== Left Sidebar Start ========== -->
		<div class="vertical-menu" style="background-color:yellow ;">
			<!-- LOGO -->
			<div class="navbar-brand-box" style="background-color:yellow ;">
				<a href="dashboard.php" ><img style="width: 60%;" src="ghj.jpg"> </a>
				<!-- <h3> M&DS</h3> -->
			</div>
			<button type="button" class="btn btn-sm px-3 font-size-16 header-item waves-effect vertical-menu-btn"> <i class="fa fa-fw fa-bars"></i> </button>
			<div data-simplebar class="sidebar-menu-scroll" style="background-color:rgb(244, 254, 67);">
				<!--- Sidemenu -->
				<div id="sidebar-menu" >
					<!-- Left Menu Start -->
			...
			<div class="page-content">
				<div class="container-fluid">
					...
					<form action="" method="POST" class="form">
						...
						<div>
							<div class="label-block"> <label for="distributor:username">Username</label> </div>
							<div class="input-box"> <input type="text" id="distributor:username" name="txtDistributorUname" placeholder="Username" value="<?php echo $usernameHolder; ?>" /> </div> <span class="error_message"><?php echo $usernameErr; ?></span>
						</div>

						<div>
							<div class="label-block"> <label for="distributor:password">Password</label> </div>
							<div class="input-box"> <input type="password" id="distributor:password" name="txtDistributorPassword" placeholder="Password" value="<?php echo $passwordHolder; ?>" /> </div> <span class="error_message"><?php echo $passwordErr; ?></span>
						</div>
